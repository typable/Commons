...
public class Updater {
	...
	public void updateDomain(String domain, String auth, String server) {
		...
		try {
			...
			if(!DOMAIN_IP_ADDRESS.equals(PUBLIC_IP_ADDRESS)) {
				...
				connection.setRequestProperty("User-Agent", "Prototype-Studio NodeOS " + Core.VERSION);
	...
	public void updateSoftware() {
		...
		try {
			...
			URL url = new URL("http://prototype-studio.de/update?version=" + Core.VERSION);
			HttpURLConnection connection = (HttpURLConnection) url.openConnection();
			connection.setRequestMethod("GET");
			...
			if(code == 200) {
				...
				String download = connection.getHeaderField("Content-Disposition");
